package com.zf.fanluxi.view.recyclerview;
...
public class FloatingItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private Map<Integer, DateHeadEntity> keys = new HashMap<>();
    ...
    private void init(Context mContext) {
        ...
        mBackgroundPaint.setColor(mContext.getResources().getColor(R.color.colorBackground));
    }
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        String price = getPrice(firstVisiblePos);
...
//                Log.e(TAG, "onDrawOver: "+child.getTop()+"$"+firstVisiblePos );
        ...
        c.drawText(price, x + 260, y, mTextPaint);
        if (flag) {
            //还原画布为初始状态
            c.restore();
        }
    ...
    private String getTitle(int position) {
        while (position >= 0) {
            if (keys.containsKey(position)) {
                return keys.get(position).getTime();
            }
            position--;
        }
        return null;
    ...
    private String getPrice(int position) {
        while (position >= 0) {
            if (keys.containsKey(position)) {
                return keys.get(position).getPrice();
            }
            position--;
        }
        return null;
    ...
    public void setKeys(Map<Integer, DateHeadEntity> keys) {
        this.keys.clear();
...
//                Log.e(TAG, "drawVertical: "+bottom );
                c.drawText(keys.get(params.getViewLayoutPosition()).getTime(), x, y, mTextPaint);

                c.drawText(keys.get(params.getViewLayoutPosition()).getPrice(), x + 260, y, mTextPaint);
            }
        }
    }
